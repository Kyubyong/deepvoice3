...
def glu(inputs):
    ...
    '''
    a, b = tf.split(inputs, 2, 2)  # (N, Tx, c) * 2
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        V = tf.get_variable('V',
                            ...
                            initializer=tf.contrib.layers.variance_scaling_initializer(
                                factor=(4. * (1. - dropout_rate))))  # (width, in_dim, out_dim)
        g = tf.get_variable('g',
                            dtype=tf.float32,
        ...
        outputs = glu(outputs)
        outputs = (outputs + _inputs) * tf.sqrt(0.5)
...
def fc_block(inputs,
    ...
    '''
    N, T, in_dim = inputs.get_shape().as_list()
    with tf.variable_scope(scope, reuse=reuse):
        inputs = tf.layers.dropout(inputs, rate=dropout_rate, training=training)
        ...
        outputs = tf.reshape(outputs, (N, T, num_units))  # (N, T, num_units)
...
def attention_block(queries,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        with tf.variable_scope("query_proj"):
            queries = fc_block(queries, hp.attention_size, training=training)  # (N, Ty/r, e)

        with tf.variable_scope("key_proj"):
            keys = fc_block(keys, hp.attention_size, training=training)  # (N, Tx, e)

        with tf.variable_scope("value_proj"):
            vals = fc_block(vals, hp.attention_size, training=training)  # (N, Tx, e)
        ...
        with tf.variable_scope("alignments"):
            ...
            if mononotic_attention:  # for inference
                key_masks = tf.sequence_mask(prev_max_attentions, hp.Tx)
                reverse_masks = tf.sequence_mask(hp.Tx - hp.attention_win_size - prev_max_attentions, hp.Tx)[:, ::-1]
                masks = tf.logical_or(key_masks, reverse_masks)
                masks = tf.tile(tf.expand_dims(masks, 1), [1, hp.Ty//hp.r, 1])
        ...
        with tf.variable_scope("context"):
            ...
            ctx = tf.matmul(ctx, vals)  # (N, Ty/r, e)
            ctx *= tf.rsqrt(tf.to_float(hp.Tx))
        ...
        alignments = tf.transpose(alignments[0])  # (Tx, Ty)
